...
function barcity(year){
    ...
    d3.json(url).then(function(data){
        ...
        for(i in data){
            cities.push(i);
            price_sqft.push(data[i]["Average (Median) Price Per Square Foot"][`${year}`]);   
        }
        ...
        var layout = {
            title: `Average Price Per Square Foot In ${year}`,
            margin: {
            l: 200,
            r: 100,
            t: 100,
...
function init(){
    ...
    d3.json("/cities").then((citiesData) => {
        ...
        //get square foot price for each city in that year
        var years= Object.keys(citiesData[city[0]]["Average (Median) Price Per Square Foot"]); 
